...
export const handler = async (event) => {
  try {
    ...
    const { consumer: user, body, path, pathParameters } = requestContext
    ...
    if (isCreatingConversationSession) {
      await doctorsService.createConversationSession(user.username, pathParameters.ticket, {
        text: !!body.text,
        video: !!body.video,
      })
    ...
    if (!isFromSameFacility && !isMaster) {
      return responseBuilder.errors.forbidden(
        'Administradores só podem criar cadástros de suas regiões'
      )
    }
