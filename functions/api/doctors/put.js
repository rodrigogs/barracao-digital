...
export const handler = async (event) => {
  try {
    ...
    const { master: wantsToBeMaster, admin: wantsTobeAdmin } = body
    ...
    if (wantsToBeMaster && !isMaster) {
      return responseBuilder.errors.forbidden(
        'Somente um usário master pode criar outro usuário master'
      )
    }

    if (wantsTobeAdmin && !isMaster) {
      return responseBuilder.errors.forbidden(
        'Somente um usuário master pode criar um administrador'
      )
    }
    ...
    if (username) {
      ...
      if (resource.endsWith('messaging/token')) {
        if (!isSelfUpdate)
          return responseBuilder.errors.forbidden('Você só pode atualizar seu próprio token')
        await doctorsService.setDoctorMessagingToken(storedDoctor, user, body)
        return responseBuilder.success.noContent()
      }

      const isTryingToChangeAnotherAdmin =
        !isSelfUpdate && (storedDoctor.admin || storedDoctor.master)
      if (isTryingToChangeAnotherAdmin && !isMaster) {
        return responseBuilder.errors.forbidden(
          'Você não pode alterar o cadástro de outros administradores'
        )
      }
