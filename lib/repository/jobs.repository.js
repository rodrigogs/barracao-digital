...
const conversationCleanupJobFunctionArn = `arn:aws:lambda:${AWS_REGION}:${AWS_ACCOUNT_ID}:function:${STACK_NAME}-${STAGE}-conversation-cleanup-job`;
...
const repository = {
  ...
  async scheduleConversationCleanupJob({ doctorUsername, patientTicket }) {
    const ruleName = `conversation-cleanup-job-${doctorUsername}-${patientTicket}`;
    console.info(`Creating rule: ${ruleName}`);
    await scheduleJob({
      ruleName,
      ...
      functionName: conversationCleanupJobFunctionArn,
      input: { doctorUsername, patientTicket },
      timeoutInMinutes: CONVERSATION_CLEANUP_TIMEOUT_IN_MINUTES,
    });
  },

  async retrieveConversationCleanupJobSchedule({ doctorUsername, patientTicket }) {
    await retrieveJobSchedule({ ruleName: `conversation-cleanup-job-${doctorUsername}-${patientTicket}` });
  },
