...
export const singleTableDriver = (tableName) => ({
  ...
  async put({ partition, sort, lock, ...attributes }) {
    ...
    const ExpressionAttributeValues = shouldLock
  ...
  async query({ partition, partitionName, sort, sortName, limit }) {
    const items = []
  ...
  queryGenerator: async function* queryGenerator({
    ...
    do {
      ;({ Items, LastEvaluatedKey } = await dynamoDB
        .query({
          TableName: tableName,
