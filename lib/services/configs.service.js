...
const service = {
  ...
  async createVideoSession(doctorUsername, patientTicket) {
    const sessionId = await configsRepository.createOpentokSession();
    const publisherToken = configsRepository.generateOpentokToken(sessionId, { role: 'publisher' });
    const subscriberToken = configsRepository.generateOpentokToken(sessionId, { role: 'subscriber' });
    const moderatorToken = configsRepository.generateOpentokToken(sessionId, { role: 'moderator' });
