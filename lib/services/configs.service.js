...
const setConfig = ({ partition, sort }, attrs) =>
  configsRepository.put({ ...attrs, partition, sort })

const getConfig = ({ partition, sort }) => configsRepository.get({ partition, sort })

const removeConfig = ({ partition, sort }) => configsRepository.remove({ partition, sort })
...
const service = {
  ...
  async setPatientMessagingToken(ticket, token) {
    const config = await getConfig({ partition: `patients#${ticket}`, sort: 'messagingToken' })
    return setConfig(
      { partition: `patients#${ticket}`, sort: 'messagingToken' },
      { ...config, token }
    )
  },
  ...
  async setDoctorMessagingToken(username, token) {
    const config = await getConfig({ partition: `doctors#${username}`, sort: 'messagingToken' })
    return setConfig(
      { partition: `doctors#${username}`, sort: 'messagingToken' },
      { ...config, token }
    )
  },
  ...
  async persistFacilityChatId(facilityOrigin, chatId) {
    return setConfig(
      {
        partition: `facilities#${facilityOrigin}`,
        sort: 'chatId',
      },
      { chatId }
    )
  ...
  async retrieveFacilityChatId(facilityOrigin) {
    return getConfig({
      partition: `facilities#${facilityOrigin}`,
      sort: 'chatId',
    })
