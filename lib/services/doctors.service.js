...
const validateDoctorPayload = (doctor) => {
  if (!doctor.fu || !doctor.crm) throw new BadRequestError('Estado e CRM são campos obrigatórios');
  if (!doctor.email || !isValidEmail(doctor.email)) throw new BadRequestError('Email inválido');
  if (!doctor.cep || !isValidCEP(doctor.cep)) throw new BadRequestError('CEP inválido');
};
