...
const checkRegionAvailability = async (cep, user) => {
  ...
  await dispatch('facilities', 'broadcastNewStatus')(cep, user, !!hasActiveDoctors);
};
...
const service = {
  ...
  async alternateActive(username, user) {
    const storedDoctor = await doctorsRepository.getOneByUsername(username);
    const updatedUser = await doctorsRepository.update({
      ...storedDoctor,
      active: !storedDoctor.active,
    });
    await checkRegionAvailability(updatedUser.cep, user);
    return updatedUser;
  },
