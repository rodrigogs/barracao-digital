...
const sendPushNotification = async (doctorUsername, { title, body }) => {
  try {
    const token = await configsService.getDoctorMessagingToken(doctorUsername);
    if (!token) return;
    const message = {
      token,
      notification: {
    ...
    await firebaseProvider.messaging.send(message);
  } catch (err) {
