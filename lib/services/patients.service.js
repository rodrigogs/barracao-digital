...
const service = {
  ...
  async update(ticket, {
    ...
    const updatingPatient = {
      ...storedPatient,
      status: status || storedPatient.status,
      lastStatus: lastStatus || storedPatient.lastStatus,
      ongoingStatus: ongoingStatus || storedPatient.ongoingStatus,
      waiting_kitStatus: waiting_kitStatus || storedPatient.waiting_kitStatus,
      finishedStatus: finishedStatus || storedPatient.finishedStatus,
      videoToken: videoToken || storedPatient.videoToken,
    };
  ...
  async setVideoSessionToken(ticket, videoToken) {
    const updatedPatient = await service.update(ticket, { videoToken });
