...
const service = {
  ...
  async setWaitingKit(patient, user = {}, { message } = {}) {
    if (!message && !patient[`${PATIENT_STATUSES.WAITING_KIT}Status`]) {
      throw new BadRequestError('O médico deve deixar instruções para o paciente ao trocar o status para "Aguardando kit"');
    }
    const patientFacility = await dispatch('facilities', 'getOneByOrigin')(patient.originCep);
  ...
  async setReceivedKit(patient, user = {}, { message } = {}) {
    if (patient.status !== PATIENT_STATUSES.WAITING_KIT) throw new ConflictError('O status do paciente não é "Aguardando kit"');
