...
const service = {
  async create(patient) {
    ...
    try {
      ...
      if (activeDoctors.items.length === 0) status = 'cant_be_assisted';
    } catch (err) {
  ...
  async setOngoing(patient, user, { message }) {
    ...
    const patientFacility = await facilitiesService.getOneByOrigin(patient.originCep);
    const updatedPatient = {
      ...patient,
      status: 'ongoing',
      ongoingFeedback: {
        ...
        facilityName: patientFacility.name,
      },
    };
