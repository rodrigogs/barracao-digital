...
const service = {
  ...
  async addOriginDestinations(origin, destinations) {
    ...
    await Promise.all(destinations // FIXME should be a batch insert
  ...
  async removeOriginDestinations(origin, destinations) {
    await Promise.all(destinations // FIXME Should be a batch delete
      .filter((destination) => destination !== origin) // Filter self destination
