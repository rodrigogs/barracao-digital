...
import facilitiesRepository from '../repository/facilities.repository'
import { PATIENT_STATUSES } from '../enums'
import { BadRequestError, ConflictError, NotFoundError } from '../errors/api'
import { isValidCEP, isValidEmail } from '../helpers/format.helper'
...
const validateFacilityPayload = (facility) => {
  ...
  if (facility.contactType === 'email' && !isValidEmail(facility.contact))
...
const getActiveFacilityPatients = async (origin) => {
  ...
  const timeWaiting = 1000 * 60 * 60 * 24
  const results = await Promise.all(
    activeStatuses.map(async (status) => {
      ...
      do {
        ;({ items, lastEvaluatedKey } = await dispatch(
          'patients',
          'getAllByOriginCepAndStatusAndTimeWaiting'
        )(origin, status, timeWaiting, { lastEvaluatedKey }))
        patients.push(...items)
...
const service = {
  ...
  async addOriginDestinations(origin, destinations) {
    ...
    if (invalidCeps.length > 0)
      throw new BadRequestError(`CEPs inválidos: ${invalidCeps.join(',')}`)
  ...
  async broadcastNewStatus(origin, isActive) {
    ...
    await Promise.all(
      facilityActivePatients.map(async (patient) => {
