...
const { ONGOING, WAITING_KIT } = require('../enums/patientStatuses.enum');
...
const service = {
  async createSession(doctorUsername, patientTicket, { text, video }) {
    ...
    if (![ONGOING, WAITING_KIT].includes(storedPatient.status)) throw new ForbiddenError('O atendimento do paciente não está em andamento');
