...
const service = {
  ...
  async createVideoSession(doctor, patient) {
    if (
      patient.status === PATIENT_STATUSES.FINISHED ||
      patient.status === PATIENT_STATUSES.CANT_BE_ASSISTED ||
      patient.status === PATIENT_STATUSES.FACILITY_NOT_AVAILABLE ||
      patient.status === PATIENT_STATUSES.GAVE_UP
    )
      return
